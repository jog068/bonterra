...
I worked decision-first: before any code, I had Claude interrogate the spec and my stack choices round by round (persistence, money representation, filter semantics, enhancement scope), so ambiguities were resolved on purpose rather than discovered mid-build. Implementation then went ticket by ticket, test-first at five pre-agreed seams: schemas, the WHERE builder, summary math, the CSV normalizer, and duplicate keys — 117 tests, each written red before the implementation. Each slice was verified in a real browser before committing.
